...
function readNumberValue(raw, fallback) {
    if (!raw) {
        return fallback;
    }
...
function parseCliArgs(argv) {
    const options = { helpRequested: false };
...
export function isHelpRequested(argv = process.argv.slice(2)) {
...
export function getRuntimeConfig(argv = process.argv.slice(2)) {
    ...
    return {
        ...
        visionBackupApiBaseUrl: readEnv('VISION_BACKUP_API_BASE_URL') || cli.apiBaseUrl || readEnv('VISION_API_BASE_URL'),
        visionBackupApiPath: readEnv('VISION_BACKUP_API_PATH') || cli.apiPath || readEnv('VISION_API_PATH') || '/v1/chat/completions',
