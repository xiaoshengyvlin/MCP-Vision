...
test('generate returns inline base64 when upstream provides b64_json', async () => {
    ...
    await withStubFetch(async (_url, init) => {
        assert.equal(JSON.parse(init.body).prompt, 'a red fox');
...
test('generate returns every image when upstream returns multiple', async () => {
    const adapter = createAdapter();
    await withStubFetch(async () => new Response(JSON.stringify({ data: [{ b64_json: 'YQ==' }, { b64_json: 'Yg==' }, { b64_json: 'Yw==' }] }), { status: 200, headers: { 'content-type': 'application/json' } }), async () => {
...
test('generate downloads a URL and converts it to base64', async () => {
    ...
    await withStubFetch(async (url) => {
        if (url.startsWith('https://example.com/v1/images')) {
            return new Response(JSON.stringify({ data: [{ url: 'https://cdn.example.com/img.png' }] }), { status: 200, headers: { 'content-type': 'application/json' } });
        }
...
test('generate does not download non-http(s) image URLs', async () => {
    ...
    await withStubFetch(async (_url, init) => {
        ...
        if (body.prompt === 'file') {
            return new Response(JSON.stringify({ data: [{ url: 'file:///C:/x.png' }] }), { status: 200, headers: { 'content-type': 'application/json' } });
        }
...
test('generate does not treat non-image content-type as a valid download', async () => {
    ...
    await withStubFetch(async (url) => {
        if (url.startsWith('https://example.com/v1/images')) {
            return new Response(JSON.stringify({ data: [{ url: 'https://cdn.example.com/error' }] }), { status: 200, headers: { 'content-type': 'application/json' } });
        }
...
test('generate surfaces upstream error.message on failure', async () => {
    const adapter = createAdapter();
    await withStubFetch(async () => new Response(JSON.stringify({ error: { message: 'billing issue' } }), {
        status: 402,
